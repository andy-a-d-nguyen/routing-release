...
Balancers and Gorouter to serve TLS traffic must contain at least one
Subject Alternative Name (SAN).


## 📑 Context

When Go 1.15 was released, the authors added a deprecation around the
...
If the foundation uses a network load balancer that includes a certificate to forward TLS traffic 
to the router, the operator *must* ensure the load balancer's certificate includes a SAN. 
Follow the procedure in [How to check a certificate for SANs](#--how-to-check-a-certificate-for-subject-alternative-names-sans) 
to validate that the certificate used contains the appropriate SAN(s).
