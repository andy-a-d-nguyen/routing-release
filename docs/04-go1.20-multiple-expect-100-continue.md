...
## 🐛 Bug 1 Summary
Previously, clients sending a request with the `Expect: 100-continue` header
received one status code 100 response before the final response.
With Go 1.20 (before the fix), clients received two status code 100 responses.
According to the HTTP/1.1 RFC, clients should handle multiple 1xx responses;
however, some Java Spring clients throw exceptions when this occurs.

## 🐞 Bug 2 Summary
Previously, when clients sent a request with the `Expect: 100-continue` header,
Gorouter's access log recorded the final response's status code, not `100`.
With Go 1.20 (before the fix), the access log showed a status code of `100`
and never logged the final status code, preventing operators from monitoring
deployment health through response codes.
...
* use other clients or frameworks that are not RFC compliant
...
## 📖 RFC Says
The RFC says that proxies like Gorouter must not filter 1xx responses. [RFC 7231, Section 6.2](https://datatracker.ietf.org/doc/html/rfc7231#section-6.2)
...
👉 This means that in Go 1.19 the reverse proxy (and thus Gorouter) was not
compliant with this spec. Now in Go 1.20 it is compliant and will forward 1xx
responses from apps.
...
1. If you are on a broken env you will see two status code 100 responses (edited response below for brevity and clarity)
   ...
   ```
1. If you are on a broken env, you will see a log with status code 100 in the app's logs
   ```
   $ cf logs potato
   ...
   ```
1. If you are on a working env, you will see one status code 100 response (edited response below for brevity and clarity)
   ```
   ...
   430387 bytes received and read
   ```
1. If you are on a working env, you will see a log with status code 200 in the app's logs
   ```
   $ cf logs potato
   Retrieving logs for app potato in org o / space s as admin...
